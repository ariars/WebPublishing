...
(function (window, $, undefined) {
    ...
    var JSGRID = "JSGrid",
        ...
        TOTAL_COUNT_PLACEHOLDER = "{totalCount}",
    ...
    function Grid(element, config) {
        ...
        var sizes = localStorage.getItem('pagersizes_' + location.pathname);
        if (sizes) {
            pagesize = sizes;
        }
    ...
    Grid.prototype = {
        ...
        _recordsTotal: function () {
            return this._loadStrategy.recordsTotal();
        },
        ...
        _createPager: function () {
            ...
            if ($.isFunction(this.pagerRenderer)) {
                ...
                //var $createPagerByFormat = this._createPagerByFormat();
                //$result = $("<div>").append($createPagerByFormat);
                ...
                var $createPagerByFormat = this._createPagerByFormat();
                if ($createPagerByFormat[0] == "<div>") {
                    $result = $("<div>");

                    $selectDiv = $("<div class='pager-count'>").append("page : ");

                    $select = $("<select class='input-sm form-control w-xxs inline v-middle'>")
                        ...
                        .on("change", this, $.proxy(function (e, h, a) {
                            localStorage.setItem('pagersizes_' + location.pathname, $(this).val());
                            e.data.pageIndex = 1;
                            e.data.pageSize = $(this).val();
                            e.data.loadData();
                            e.data._resetPager();
                        }));
                    var sizes = localStorage.getItem('pagersizes_' + location.pathname);
                    ...
                    var $div = null;
                    $($createPagerByFormat).each(function () {
                        if (this == "<div>") {
                            $div = $("<div>").addClass("paginate");
                        }
                        else if (this == "</div>") {
                            $result.append($div);
                        }
        ...
        _createPagerByFormat: function () {
            ...
            return $.map(pagerParts, $.proxy(function (pagerPart) {
                ...
                if (pagerPart === PAGES_PLACEHOLDER) {
                    ...
                    result = itemCount;
                } else if (pagerPart === TOTAL_COUNT_PLACEHOLDER) {
                    if (recordsTotal == -1) {
                        result = itemCount + " over";
                    }
                    else {
                        result = recordsTotal;
                    }
        ...
        _createPagerButton: function (text, css, handler) {
            ...
            //    .on("click", $.proxy(handler, this));
            //if (text == "First")
            //    $link.addClass("first").prepend("<img src = '/Content/Images/button/btn_pn_first.gif' />");
            //else if (text == "Last")
            //    $link.addClass("last").prepend("<img src = '/Content/Images/button/btn_pn_last.gif' />");
            //else if (text == "Next")
            //    $link.addClass("next").prepend("<img src = '/Content/Images/button/btn_pn_next.gif' />");
            //else if (text == "Prev")
            //    $link.addClass("pre").prepend("<img src = '/Content/Images/button/btn_pn_pre.gif' />");
            //else
            //    $link.html(text).addClass(css)
...
(function (jsGrid, $, undefined) {
    ...
    PageLoadingStrategy.prototype = {
        ...
        finishLoad: function (loadedData) {
            ...
            this._recordsTotal = loadedData.recordsTotal;
...
(function (jsGrid, $, undefined) {
    ...
    TextField.prototype = new Field({
        ...
        _createTextBox: function () {
            return $("<input>").attr("type", "text").attr("class", "jstb" + this.name + " form-control")
...
(function (jsGrid, $, undefined) {
    ...
    NumberField.prototype = new TextField({
        ...
        _createTextBox: function () {
            return $("<input>").attr("type", "number").attr("class", "jstb" + this.name + " form-control")
                .prop("readonly", !!this.readOnly);
        }
    });
...
(function (jsGrid, $, undefined) {
    ...
    CheckboxField.prototype = new Field({
        ...
        editTemplate: function (value) {
            ...
            $result.find("input").prop("checked", value);
            return $result;
        },
        ...
        insertValue: function () {
            return this.insertControl.find("input").is(":checked");
        },

        editValue: function () {
            return this.editControl.find("input").is(":checked");
        },

        _createCheckbox: function () {
            return $("<div class=\"checkbox\"><label class=\"i-checks\"><input type=\"checkbox\" value=\"\" checked><i></i></label>");
        }
    });
...
(function (jsGrid, $, undefined) {
    ...
    ControlField.prototype = new Field({
        ...
        buttonClass: "button03 pr small non-event",
        ...
        editButtonTooltip: LABEL.Modify,
        deleteButtonTooltip: LABEL.Delete,
        insertButtonTooltip: LABEL.Insert,
        updateButtonTooltip: LABEL.Save,
        cancelEditButtonTooltip: LABEL.Cancel,
        cancelInsertButtonTooltip: LABEL.Cancel,
        ...
        _createDeleteButton: function (item) {
            var btn = this._createGridButton(this.deleteButtonClass, this.deleteButtonTooltip, function (grid, e) {
                grid.deleteItem(item);
                e.stopPropagation();
            }, "ico_s_del");

            return btn.attr("style", "margin-left:5px");
        },
        ...
        _createCancelEditButton: function () {
            var btn = this._createGridButton(this.cancelEditButtonClass, this.cancelEditButtonTooltip, function (grid, e) {
                grid.cancelEdit();
                e.stopPropagation();
            }, "ico_cancel");
        ...
        _createCancelInsertButton: function () {
            var btn = this._createGridButton(this.cancelInsertButtonClass, this.cancelInsertButtonTooltip, function (grid, e) {
                grid.clearInsert();
        ...
        _createGridButton: function (cls, tooltip, clickHandler, iconcls) {
            var grid = this._grid;

            var btn = $("<button>").addClass(this.buttonClass)
                .addClass(cls)
                .attr({
                    type: "button",
                    title: tooltip,
            ...
            if (iconcls) {
                btn.append($("<span/>").addClass(iconcls))
            }

            return btn.append(tooltip);
        },
...
(function (jsGrid, $, undefined) {
    ...
    var PopUpField = function (config) {
    ...
    PopUpField.prototype = new jsGrid.Field({
        ...
        popupTemplate: function (grid) {
            var $result = this._createPopupBox();
            var $input = $($result).find("input[type=text]");
            return $input;
        },
        ...
        filterTemplate: function () {
            ...
            var grid = this._grid,
                $result = this.filterControl = this._createPopupBox();
        ...
        insertTemplate: function () {
            ...
            return this.insertControl = this._createPopupBox();
        },

        editTemplate: function (text) {
            if (!this.editing)
                return this.itemTemplate.apply(this, arguments);
            ...
            var $result = this.editControl = this._createPopupBox();
            $result.find(".valueField").val(v);
            $result.find(".textField").val(text);
            return $result;
        },

        filterValue: function () {
            return this.filterControl.find(".valueField").val();
        },

        insertValue: function () {
            return this.insertControl.find(".valueField").val();
        },

        editValue: function () {
            return this.editControl.find(".valueField").val();
        },

        _createPopupBox: function () {
            callback = this.callback;
            var input = $("<input>").attr("type", "text").attr("class", "textField form-control").attr("readonly", true);
            var hiddne = $("<input>").attr("type", "hidden").attr("class", "valueField");
            var button = $("<button>").attr("type", "button").attr("class", "button03 pr non-event").text(LABEL.Search)
                .on("click", $.proxy(function (e) {
                    callback(e);
                }, this));

            return $("<div style='display: flex;'>").append(input).append(hiddne).append(button);
        }
    });

    jsGrid.fields.popup = PopUpField;
...
(function (jsGrid, $, undefined) {
    ...
    function SelectChangeField(config) {
        this.items = [];
        this.selectedIndex = -1;
        this.valueField = "";
        this.textField = "";
    ...
    SelectChangeField.prototype = new NumberField({
        ...
        _createSelect: function () {
            var $result = $("<select class='jssb" + this.name + "'>"),
                valueField = this.valueField,
                textField = this.textField,
                selectedIndex = this.selectedIndex,
                callback = this.callback;

            $("<option>").attr("value", "0").text(MESSAGE.Select).appendTo($result);
    ...
    jsGrid.fields.selectchange = jsGrid.SelectChangeField = SelectChangeField;
